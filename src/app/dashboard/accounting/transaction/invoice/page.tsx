    ...
    <div className="space-y-3">
      ...
      <div className="flex flex-col lg:flex-row justify-between items-start lg:items-center gap-4 mb-6">
        ...
        <div className="flex items-center gap-3">
          ...
          <button className="px-4 py-2 border border-gray-300 text-gray-700 rounded-lg hover:bg-gray-300 transition-colors flex items-center gap-2">
        ...
        <div className="bg-white rounded-lg shadow-sm border border-gray-200">
              ...
              <table className="w-full min-w-[800px]">
                <thead className="bg-gray-50 top-0 border-b border-gray-200">
                  <tr>
                    <th className="text-left p-3 md:p-4 font-medium text-gray-700 w-12">
                      <input type="checkbox" className="rounded border-gray-300 text-pink-500 focus:ring-pink-500" />
                    </th>
                    <th className="text-left p-3 md:p-4 font-medium text-gray-700 text-sm">Invoice date</th>
                    <th className="text-left p-3 md:p-4 font-medium text-gray-700 text-sm">Invoice number#</th>
                    <th className="text-left p-3 md:p-4 font-medium text-gray-700 text-sm">Order id#</th>
                    <th className="text-left p-3 md:p-4 font-medium text-gray-700 text-sm">Status</th>
                    <th className="text-left p-3 md:p-4 font-medium text-gray-700 text-sm">Customer Name</th>
                    <th className="text-left p-3 md:p-4 font-medium text-gray-700 text-sm">Due date</th>
                    <th className="text-left p-3 md:p-4 font-medium text-gray-700 text-sm">Balance</th>
                    <th className="text-left p-3 md:p-4 font-medium text-gray-700 text-sm">Branch</th>
                  </tr>
                </thead>
                <tbody className="divide-y divide-gray-100">
                  {invoices.map((invoice, index) => (
                    <tr key={index} className="hover:bg-gray-50">
                      ...
                      <td className="p-3 md:p-4 text-sm text-gray-900 whitespace-nowrap">{invoice.date}</td>
                      <td className="p-3 md:p-4 whitespace-nowrap">
                        <span className="text-sm text-blue-600 hover:text-blue-800 cursor-pointer font-medium">
                          {invoice.number}
                        </span>
                      </td>
                      <td className="p-3 md:p-4 text-sm text-gray-600 whitespace-nowrap">{invoice.orderId}</td>
                      <td className="p-3 md:p-4 whitespace-nowrap">
                        <span className={`text-sm font-medium ${getStatusColor(invoice.status)}`}>
                          {invoice.status}
                        </span>
                      </td>
                      <td className="p-3 md:p-4 text-sm text-gray-900 whitespace-nowrap">{invoice.customer}</td>
                      <td className="p-3 md:p-4 text-sm text-gray-900 whitespace-nowrap">{invoice.dueDate}</td>
                      <td className="p-3 md:p-4 text-sm text-gray-900 font-medium whitespace-nowrap">{invoice.balance}</td>
                      <td className="p-3 md:p-4 text-sm text-gray-900 whitespace-nowrap">{invoice.branch}</td>
                    </tr>
                  ))}
                </tbody>
              </table>
          ...
          <div className="flex flex-col sm:flex-row sm:justify-between sm:items-center px-4 md:px-6 py-4 border-t border-gray-200 bg-white gap-3">
            <button className="px-4 py-2 text-gray-600 hover:text-gray-800 font-medium text-center sm:text-left">
              Previous
            </button>
            <span className="text-sm text-gray-600 text-center">Page 1 of 10</span>
            <button className="px-4 py-2 text-gray-600 hover:text-gray-800 font-medium text-center sm:text-right">
              Next
            </button>
          </div>
        </div>
